...
class FeatureExtractor(FeatureWorker):
	...
	print "correl_field\t", self.correl_field
	#self.correl_field='message_id'
        #Create Table:
    ...
    def _countFeatures(self, collocSet, maxCollocSizeByFirstWord, message, tokenizer, freqs, includeSubCollocs=False):
        '''
        :param collocSet: set - set of features we will extract
            ?can we assume all features are separated by a space?
        :param message: text from which we would like to extract features
        :param tokenizer: an object with method tokenize() that returns words from a piece of text
        :param freqs: running count of how many times each word is used
        '''
    ...
    def addCollocFeatTable(self, collocList, min_freq=1, tableName = None, valueFunc = lambda d: d, includeSubCollocs=False, featureTypeName=None):
        """
        :param collocs: - may be a list or a dict
        """
        """Creates feature tuples (correl_field, feature, values) table where features are ngrams"""
        """Optional ValueFunc program scales that features by the function given"""
        ##NOTE: correl_field should have an index for this to be quick
    ...
    #def addLexiconFeat(self, lexiconTableName, tableName=None, valueFunc = lambda x: x, isWeighted=False, featValueFunc=lambda d: log2(float(d)+1)):     #16to4
    #def addLexiconFeat(self, lexiconTableName, tableName=None, valueFunc = lambda x: x, isWeighted=False, featValueFunc=lambda d: float(1)):          #16to1
...
###################################################################""")
    ...
    def addWNNoPosFeat(self, tableName=None, valueFunc = lambda x: float(x), featValueFunc=lambda d: float(d)):          #16to16
    ...
    def addWNPosFeat(self, tableName=None, pos_table = None, valueFunc = lambda x: float(x), featValueFunc=lambda d: float(d)):          #16to16
